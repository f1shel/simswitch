...
public class Invariant : MonoBehaviour
{
    float dt = 0.003f;
    float mass = 1;
    float lame_lambda = 20000.0f;
    float lame_mu = 5000.0f;
    float damp = 0.999f;
    ...
    Matrix4x4 MaddM(Matrix4x4 m, Matrix4x4 n)
    {
        Matrix4x4 c = Matrix4x4.identity;
        c.SetColumn(0, m.GetColumn(0) + n.GetColumn(0));
        c.SetColumn(1, m.GetColumn(1) + n.GetColumn(1));
        c.SetColumn(2, m.GetColumn(2) + n.GetColumn(2));
        return c;
    }

    Matrix4x4 MsubM(Matrix4x4 m, Matrix4x4 n)
    {
        Matrix4x4 c = Matrix4x4.identity;
        c.SetColumn(0, m.GetColumn(0) - n.GetColumn(0));
        c.SetColumn(1, m.GetColumn(1) - n.GetColumn(1));
    ...
    Matrix4x4 Maddf(Matrix4x4 m, float n)
    {
        Matrix4x4 c = Matrix4x4.identity;
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
    ...
    Matrix4x4 Msubf(Matrix4x4 m, float n)
    {
        Matrix4x4 c = Matrix4x4.identity;
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
    ...
    Matrix4x4 Mmulf(Matrix4x4 m, float n)
    {
        Matrix4x4 c = Matrix4x4.identity;
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
    ...
    void _Update()
    {
        ...
        for (int tet = 0; tet < tet_number; tet++)
        {
            ...
            Matrix4x4 C_inv = C.inverse;
            Matrix4x4 I = Matrix4x4.identity;
            Matrix4x4 S = MaddM(Mmulf(MsubM(I, C_inv), lame_mu), Mmulf(C_inv, lame_lambda * Mathf.Log(F.determinant, 2)));
